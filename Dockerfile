...
RUN mkdir -p "$I2PD_HOME" "$DATA_DIR" \
    && { if [ "$UID" -ne "0" ]; then \
              addgroup -S -g $GID i2pd \
           && adduser -S -u $UID -G i2pd -h "$I2PD_HOME" i2pd \
           && chown -R i2pd:i2pd "$I2PD_HOME"; \
         else \
              [ "$GID" -eq "0" ] \
           && printf 'i2pd:x:%d:i2pd\n' $GID >> /etc/group \
           && printf 'i2pd:x:%d:%d:i2pd:%s:/bin/sh\n' $UID $GID "$I2PD_HOME" >> /etc/passwd \
           && mkdir -p "$I2PD_HOME" \
           && chown -R $UID:$GID "$I2PD_HOME"; \
         fi; \
       }
